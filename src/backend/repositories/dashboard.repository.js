...
WHERE regions.display_name LIKE '%Garut%'
ORDER BY
...
export function getProvinceRows(db) {
  return db
    .prepare(
      ...
      FROM provinces
      INNER JOIN province_metrics 
        ON province_metrics.province_key = provinces.province_key
      WHERE provinces.display_name LIKE '%Jawa Barat%'
      ORDER BY
...
AND packages.location_raw LIKE '%Garut%'
    `
    )
